...
void BobikCasters::serial_message_handler(unsigned char* data, uint8_t *log_buf)
{
    ...
    digitalWrite(caster_settings->fl_rot.in1, RobotUtils::sign1(desired_rot));
    digitalWrite(caster_settings->fl_rot.in2, RobotUtils::sign2(desired_rot));
    analogWrite(caster_settings->fl_rot.ena, desired_rot);
    digitalWrite(caster_settings->fl_drive.in1, RobotUtils::sign1(desired_drive));
    digitalWrite(caster_settings->fl_drive.in2, RobotUtils::sign2(desired_drive));
    analogWrite(caster_settings->fl_drive.ena, desired_drive);

    digitalWrite(caster_settings->fr_rot.in1, RobotUtils::sign1(desired_rot));
    digitalWrite(caster_settings->fr_rot.in2, RobotUtils::sign2(desired_rot));
    analogWrite(caster_settings->fr_rot.ena, desired_rot);
    digitalWrite(caster_settings->fr_drive.in1, RobotUtils::sign1(desired_drive));
    digitalWrite(caster_settings->fr_drive.in2, RobotUtils::sign2(desired_drive));
    analogWrite(caster_settings->fr_drive.ena, desired_drive);

    digitalWrite(caster_settings->r_rot.in1, RobotUtils::sign1(desired_rot));
    digitalWrite(caster_settings->r_rot.in2, RobotUtils::sign2(desired_rot));
    analogWrite(caster_settings->r_rot.ena, desired_rot);
    digitalWrite(caster_settings->r_drive.in1, RobotUtils::sign1(desired_drive));
    digitalWrite(caster_settings->r_drive.in2, RobotUtils::sign2(desired_drive));
